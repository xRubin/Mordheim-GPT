...
use Mordheim\Classic\Rule\RecoveryPhase;
...
class Battle
{
    ...
    {
        BattleLogger::add("Фаза движения: {$band->name}");
    ...
    public function runAwayInPanic(Fighter $fighter): void
    {
        ...
        if ($target === null) {
            BattleLogger::add("{$fighter->getName()} не может убежать в панике: все края заняты.");
            return;
        }

        try {
            Move::run($this, $fighter, $target, 0.4); // минимальная агрессивность
        } catch (PathfinderTargetUnreachableException $e) {
            BattleLogger::add("{$fighter->getName()} не может убежать в панике: путь до края недоступен (" . implode(', ', $fighter->getState()->getPosition()) . ") -> (" . implode(', ', $target) . ")");
        }
    }
    ...
    {
        BattleLogger::add("Фаза стрельбы: {$band->name}");
        foreach ($band->fighters as $fighter) {
    ...
    {
        BattleLogger::add("Фаза магии: {$band->name}");
        foreach ($band->fighters as $fighter) {
            if ($fighter->getState()->getStatus()->canAct()) {
                $enemies = $this->getEnemiesFor($fighter);
    ...
    {
        BattleLogger::add("Фаза рукопашного боя");
        foreach ($this->getActiveCombats()->getAll() as $combat) {
            BattleLogger::add("[DEBUG][CloseCombat] бой (" . implode(', ', array_map(fn($fighter) => $fighter->getName(), $combat->fighters)) . ")");
