...
class RecoveryPhase
{
    ...
    public static function applyPsychology(Battle $battle, Fighter $fighter, Band $band, array $warbands): bool
    {
        ...
        // --- PANIC recovery ---
        if ($fighter->getState()->getStatus() === Status::PANIC) {
            if (Psychology::leadershipTest($fighter, $band->fighters)) {
                $fighter->getState()->setStatus(Status::STANDING);
                BattleLogger::add("{$fighter->getName()} преодолел панику и возвращается в бой!");
                // TODO: в этот ход может только колдовать
            } else {
                BattleLogger::add("{$fighter->getName()} всё ещё в панике!");
                return false;
            }
        }
        // --- All Alone ---
        if ($fighter->getState()->getStatus() === Status::STANDING) {
            ...
            if (count($closeEnemies) >= 2 && count($closeAllies) === 0) {
                if (!Psychology::allAloneTest($fighter, $enemies, $allies)) {
                    $fighter->getState()->setStatus(Status::PANIC);
                    BattleLogger::add("{$fighter->getName()} не выдержал одиночества и впадает в панику!");
                    return false;
                }
            }
        }
        // --- Stupidity ---
        if ($fighter->hasSpecialRule(SpecialRule::STUPIDITY) && $fighter->getState()->getStatus() === Status::STANDING) {
            if (!Psychology::leadershipTest($fighter, $band->fighters)) {
                BattleLogger::add("{$fighter->getName()} не прошёл тест тупости и стоит без дела!");
                return false;
            } else {
                BattleLogger::add("{$fighter->getName()} прошёл тест тупости и может действовать нормально.");
            }
        }
        // --- Fear ---
        $enemies = [];
