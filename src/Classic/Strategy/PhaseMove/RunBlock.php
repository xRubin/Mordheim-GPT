...
class RunBlock
{
    ...
    public function __invoke()
    {
        return function (Battle $battle, Fighter $fighter): bool {
            ...
            try {
                Move::runIfNoEnemies($battle, $fighter, $target->getState()->getPosition(), $this->aggressiveness);
                $this->strategy->spentShoot = true;
                $this->strategy->spentMove = true;
                return true;
            } catch (PathfinderTargetUnreachableException|MoveRunDeprecatedException $e) {
                return false;
            }
        };
    }
