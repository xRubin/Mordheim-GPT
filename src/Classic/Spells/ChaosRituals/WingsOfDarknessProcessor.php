...
class WingsOfDarknessProcessor extends BaseSpellProcessor
{
    public Spell $spell = Spell::WINGS_OF_DARKNESS;
    ...
    public function onPhaseMagic(Battle $battle, Fighter $caster): ?bool
    {
        ...
        if ($target->getState()->getStatus() === Status::PANIC) {
            BattleLogger::add("{$caster->getName()} наносит автоматический удар по бегущему {$target->getName()} ({$this->spell->name}).");
