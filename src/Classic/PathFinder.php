...
class PathFinder
{
    ...
    public static function canClimb(GameField $field, array $start, array $goal): bool
    {
        ...
        if ($field->getCell($nx, $ny, $nz) && $field->getCell($nx, $ny, $nz)->obstacle) return false;
        ...
        if (!($x + 1 == $nx && $y == $ny && $field->getCell($x + 1, $y, $z) && $field->getCell($x + 1, $y, $z)->obstacle)
            && !($x - 1 == $nx && $y == $ny && $field->getCell($x - 1, $y, $z) && $field->getCell($x - 1, $y, $z)->obstacle)
            && !($y + 1 == $ny && $x == $nx && $field->getCell($x, $y + 1, $z) && $field->getCell($x, $y + 1, $z)->obstacle)
            && !($y - 1 == $ny && $x == $nx && $field->getCell($x, $y - 1, $z) && $field->getCell($x, $y - 1, $z)->obstacle))
            return false;
        // В ячейке бойца не должно быть препятствия
        if ($field->getCell($x, $y, $z) && $field->getCell($x, $y, $z)->obstacle) return false;
    ...
    public static function canJumpOverGap(GameField $field, array $start, array $goal): bool
    {
        ...
        if ($dx === 0) {
            for ($i = min($y, $ny); $i <= max($y, $ny); $i++)
                if ($field->getCell($x, $i, $z) && $field->getCell($x, $i, $z)) return false;
        }
        if ($dy === 0) {
            for ($i = min($x, $nx); $i <= max($x, $nx); $i++)
                if ($field->getCell($i, $y, $z) && $field->getCell($i, $x, $z)->obstacle) return false;
        }
        return true;
    }
